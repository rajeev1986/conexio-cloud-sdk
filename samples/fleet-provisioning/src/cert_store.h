...
/**
 * @brief Clear any stale device credentials from modem secure storage.
 *
 * Deletes device cert (tag 21) and device key (tag 22) if present.
 * Safe to call when the slots are already empty.
 * Call this before starting a fresh provisioning attempt to ensure
 * no partial credentials from a previous failed run interfere.
 *
 * @return 0 always (delete errors are logged as warnings, not fatal).
 */
 ...
 *
 * Always deletes the existing credential before writing so that stale
 * entries from failed provisioning attempts never cause -EACCES errors.
 *
