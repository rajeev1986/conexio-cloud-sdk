...
/* CreateKeysAndCertificate response contains cert PEM (~1.7 KB) +
 * private key PEM (~1.7 KB) + ownership token (~0.5 KB) + JSON overhead.
 * 6 KB gives comfortable headroom. */
#define PAYLOAD_BUF_SIZE 6144
                                  ...
                                  const struct mqtt_evt *evt)
{
    ...
    case MQTT_EVT_CONNACK:
        if (evt->result == 0) {
            ...
            /* Subscribe first — wait for SUBACK before publishing.
             * This ensures we are in the poll loop and ready to receive
             * the CreateKeysAndCertificate response before AWS sends it. */
            ...
            /* CreateKeysAndCertificate publish happens in MQTT_EVT_SUBACK */
        } else {
    ...
    case MQTT_EVT_PUBLISH: {
        ...
        LOG_INF("MQTT_EVT_PUBLISH: topic='%s' payload_len=%zu", topic, payload_len);

        /* Read payload — use full buffer size, AWS cert response is ~3.5 KB */
        size_t read_len = MIN(payload_len, sizeof(payload_buf) - 1);
        if (payload_len > sizeof(payload_buf) - 1) {
            LOG_WRN("Payload %zu bytes exceeds buffer %zu — truncating",
                    payload_len, sizeof(payload_buf) - 1);
        }
        ret = mqtt_read_publish_payload_blocking(client, payload_buf, read_len);
        if (ret < 0) {
            LOG_ERR("mqtt_read_publish_payload failed (err %d)", ret);
            /* Must still drain remaining bytes or MQTT stream gets out of sync */
            break;
        }
        payload_buf[ret] = '\0';
        LOG_INF("Payload read: %d bytes", ret);
    ...
    case MQTT_EVT_SUBACK:
        LOG_INF("SUBACK — provisioning topics subscribed, publishing CreateKeysAndCertificate...");
        /* Now safe to publish — we are subscribed and back in the poll loop */
...
int run_provisioning(const char *device_id, struct mqtt_client *client)
{
    ...
    /* Drive MQTT until done or timeout (3 minutes).
     * AWS IoT TLS handshake on cellular can take 30-60s on first connect. */
    int64_t deadline_ms = k_uptime_get() + 180000;

    while (prov_state != PROV_STATE_DONE && prov_state != PROV_STATE_FAILED) {
        if (k_uptime_get() > deadline_ms) {
            LOG_ERR("Fleet Provisioning timed out after 3 minutes");
