...
/* ── MQTT buffers ─────────────────────────────────────────────────────────── */
/* AWS IoT TLS CONNACK + provisioning responses (cert PEM ~1.7 KB + key ~1.7 KB)
 * need larger buffers than a typical MQTT broker.
 * rx_buf must be >= largest single MQTT packet AWS will send. */
static uint8_t mqtt_rx_buf[6144];
...
/* NCS v3.2.1: h_set callback takes 4 args (no trailing void *param) */
...
static int derive_device_id(void)
{
     ...
     * This works as soon as nrf_modem_lib is initialised — no LTE needed.
     * modem_info_params_get() requires network params and returns a zeroed
     * IMEI field when the device is not yet registered. */
    int ret = modem_info_string_get(MODEM_INFO_IMEI, imei, sizeof(imei));
    if (ret < 0) {
        LOG_ERR("modem_info_string_get(IMEI) failed (err %d)", ret);
        return ret;
    }
    ...
    if (strlen(imei) == 0) {
        LOG_ERR("IMEI string is empty after modem_info_string_get");
        return -ENODATA;
    }
...
/* Return int so error paths can propagate; caller ignores the value. */
static int reboot_counter_init(void)
{
...
/* ISO-8601 timestamp buffer size: "2025-06-13T12:34:56.789Z" = 24 chars + NUL.
 * Using 32 gives headroom and silences -Wformat-truncation. */
#define ISO8601_BUF_LEN 32
...
static void get_iso8601(char *buf, size_t buf_len)
{
    ...
    if (ret) {
        /* NTP not synced yet — use uptime as HH:MM:SS fallback */
        ...
        snprintf(buf, buf_len, "1970-01-01T%02u:%02u:%02u.000Z", hh, mm, ss);
        return;
    }
    time_t unix_sec = (time_t)(unix_ms / 1000);
    struct tm *t = gmtime(&unix_sec);
    unsigned ms = (unsigned)(unix_ms >= 0 ? unix_ms % 1000 : 0);

    /* Build ISO-8601 with direct digit writes — avoids -Wformat-truncation.
     * Output is always exactly 24 chars: "YYYY-MM-DDTHH:MM:SS.mmmZ" */
    int year = t->tm_year + 1900;
    if (year < 0)    { year = 0;    }
    if (year > 9999) { year = 9999; }

    char tmp[25]; /* 24 chars + NUL */
    tmp[ 0] = '0' + (year / 1000) % 10;
    tmp[ 1] = '0' + (year /  100) % 10;
    tmp[ 2] = '0' + (year /   10) % 10;
    tmp[ 3] = '0' + (year       ) % 10;
    tmp[ 4] = '-';
    tmp[ 5] = '0' + ((t->tm_mon + 1) / 10) % 10;
    tmp[ 6] = '0' + ((t->tm_mon + 1)     ) % 10;
    tmp[ 7] = '-';
    tmp[ 8] = '0' + (t->tm_mday / 10) % 10;
    tmp[ 9] = '0' + (t->tm_mday     ) % 10;
    tmp[10] = 'T';
    tmp[11] = '0' + (t->tm_hour / 10) % 10;
    tmp[12] = '0' + (t->tm_hour     ) % 10;
    tmp[13] = ':';
    tmp[14] = '0' + (t->tm_min / 10) % 10;
    tmp[15] = '0' + (t->tm_min     ) % 10;
    tmp[16] = ':';
    tmp[17] = '0' + (t->tm_sec / 10) % 10;
    tmp[18] = '0' + (t->tm_sec     ) % 10;
    tmp[19] = '.';
    tmp[20] = '0' + (ms / 100) % 10;
    tmp[21] = '0' + (ms /  10) % 10;
    tmp[22] = '0' + (ms      ) % 10;
    ...
    size_t copy = buf_len > sizeof(tmp) ? sizeof(tmp) : buf_len;
    memcpy(buf, tmp, copy);
    buf[copy - 1] = '\0'; /* ensure NUL even if buf_len < 25 */
}
...
{
    char timestamp[ISO8601_BUF_LEN];
...
{
    /* NCS v3.2.1: use zsock_addrinfo / zsock_getaddrinfo directly.
     * addrinfo / getaddrinfo aliases require CONFIG_POSIX_API=y which pulls
     * in the full POSIX layer — unnecessary for an nRF91xx offloaded socket. */
...
int main(void)
{
    ...
    /* 2. Initialise the modem library and put modem in offline mode so that
     *    modem_key_mgmt can write credentials over the AT interface.
     *    nrf_modem_lib_init() is idempotent — safe to call every boot.
     *    lte_lc_func_mode_set(OFFLINE) ensures the modem is not registered
     *    on the network while we write to secure storage. */
    ret = nrf_modem_lib_init();
    if (ret) {
        LOG_ERR("nrf_modem_lib_init failed (err %d)", ret);
        return -1;
    }
    ret = lte_lc_func_mode_set(LTE_LC_FUNC_MODE_OFFLINE);
    if (ret) {
        LOG_WRN("lte_lc_func_mode_set(OFFLINE) failed (err %d) — continuing", ret);
    }

    /* 3. Provision claim credentials into modem (safe to call every boot) */
    ...
    /* 4. Init modem info and derive device ID from IMEI.
     *    modem_info_string_get(MODEM_INFO_IMEI) issues AT+CGSN directly —
     *    works immediately after nrf_modem_lib_init(), no LTE needed. */
    modem_info_init();
    ret = derive_device_id();
    if (ret) {
        LOG_ERR("Failed to derive device ID from IMEI (err %d) — halting", ret);
        return -1;
    }

    /* 5. Initialise reboot counter (NVS) */
    reboot_counter_init();
    ...
    /* 8. Resolve broker once (same host for both phases) */
        ...
        if (ret == 0) {
            ...
            while (k_uptime_get() < uart_deadline && !uart_done) {
                uart_comm_status_t status =
                    uart_commissioning_poll(g_device_id, CONFIG_CONEXIO_FW_VERSION);
    ...
    if (!already_provisioned) {
        ...
        /* Clear any stale device credentials from previous failed attempts.
         * modem_key_mgmt_write() returns -EACCES if a slot is already
         * occupied on some modem FW versions. Deleting first guarantees
         * a clean write regardless of what was left from the last run. */
        cert_store_clear_device_creds();

        /* Also clear the persisted provisioning flag in case it was set
         * inconsistently (flag=true but device certs missing or corrupt). */
        settings_delete(SETTINGS_KEY);
        g_provisioning_done = false;
