...
/* Delete a credential if it exists — ignores "not found" errors. */
static void delete_if_exists(int tag,
                             enum modem_key_mgmt_cred_type type,
                             const char *label)
{
    bool exists = false;
    ...
    ret = modem_key_mgmt_delete(tag, type);
    if (ret < 0) {
        LOG_WRN("Failed to delete %s (tag %d, err %d) — will attempt overwrite", label, tag, ret);
    } else {
        LOG_INF("Deleted stale %s (tag %d)", label, tag);
    }
}
...
int cert_store_clear_device_creds(void)
{
    LOG_INF("Clearing any stale device credentials from modem...");
    delete_if_exists(DEVICE_CERT_TAG, MODEM_KEY_MGMT_CRED_TYPE_PUBLIC_CERT,  "device cert");
    delete_if_exists(DEVICE_KEY_TAG,  MODEM_KEY_MGMT_CRED_TYPE_PRIVATE_CERT, "device key");
    LOG_INF("Device credential slots cleared (tags %d/%d)",
            DEVICE_CERT_TAG, DEVICE_KEY_TAG);
    return 0;
}
...
int cert_store_write_device_creds(const char *cert, const char *key)
{
    ...
    /* Always delete before writing — modem_key_mgmt_write() on a slot that
     * already contains a credential returns -EACCES on some modem FW versions.
     * Deleting first guarantees a clean write regardless of prior state. */
    delete_if_exists(DEVICE_CERT_TAG, MODEM_KEY_MGMT_CRED_TYPE_PUBLIC_CERT,  "device cert");
    delete_if_exists(DEVICE_KEY_TAG,  MODEM_KEY_MGMT_CRED_TYPE_PRIVATE_CERT, "device key");
