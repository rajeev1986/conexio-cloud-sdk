 ...
 *
 * Collects LTE neighbor cell data via AT%NCELLMEAS and publishes it as
 ...
 *   _loc_rsrp         RSRP index of serving cell           (int)
 *                     Convert to dBm: RSRP_IDX_TO_DBM(idx)
 *   _loc_timing_adv   Timing advance (distance proxy)      (int, optional)
 *   _loc_neighbors    JSON array of neighbor cells         (string)
 *                     [{"earfcn":5110,"pci":42,"rsrp":55},...]
 *
 ...
 *
 * Power note:
 *   AT%NCELLMEAS keeps the radio active for 1–5 seconds. Do not call it
 *   more often than needed — on a 60s publish interval, once per publish
 *   is fine for most tracking use cases. For ultra-low-power deployments,
 *   consider calling it only every N publishes (e.g. every 5 minutes).
 */
...
/* ── RSRP raw index → dBm ─────────────────────────────────────────────
 * Per 3GPP TS 36.133: RSRP_dBm = index - 140  (for positive indices)
 * or                  RSRP_dBm = index + 140  (for negative, per nRF AT ref)
 * Using the same macro as NCS: RSRP_IDX_TO_DBM is defined in lte_lc.h.
 */
#ifndef RSRP_IDX_TO_DBM
#  define RSRP_IDX_TO_DBM(x) ((x) < 0 ? (x) - 140 : (x) - 141)
#endif

/* ── Serialize neighbor cells to compact JSON string ───────────────────
 *
 * Format: [{"earfcn":5110,"pci":42,"rsrp":-85},{"earfcn":5110,"pci":17,"rsrp":-90}]
 *
 * Kept compact (no whitespace) to minimize telemetry payload size.
 * Lambda parses this as a JSON array for the positioning API call.
 *
 * @param neighbors   Pointer to neighbor cell array
                                 ...
                                 char *buf, size_t buf_size)
{
    ...
    for (uint8_t i = 0; i < count; i++) {
        ...
        if (!cell) { cJSON_Delete(arr); return false; }

        cJSON_AddNumberToObject(cell, "earfcn", (double)n->earfcn);
        cJSON_AddNumberToObject(cell, "pci",    (double)n->phys_cell_id);
        /* Convert RSRP index to dBm for the Lambda */
        if (n->rsrp != LTE_LC_CELL_RSRP_INVALID) {
            cJSON_AddNumberToObject(cell, "rsrp", (double)RSRP_IDX_TO_DBM(n->rsrp));
        }
    ...
    if (ok) {
        ...
        LOG_WRN("cell_location: neighbor buffer too small (%zu < %zu) — "
...
static void process_cell_measurement(const struct lte_lc_cells_info *cells)
{
    ...
    /* RSRP: send the raw index — Lambda converts to dBm if needed,
     * but also send the dBm value for direct use. */
    if (sc->rsrp != LTE_LC_CELL_RSRP_INVALID) {
        conexio_cloud_send_metric("_loc_rsrp", (double)RSRP_IDX_TO_DBM(sc->rsrp));
    }

    /* Timing advance — useful for distance estimation (~100m resolution).
     * Only send when valid (INVALID means the modem doesn't have a fresh value). */
    if (sc->timing_advance != LTE_LC_CELL_TIMING_ADVANCE_INVALID) {
        conexio_cloud_send_metric("_loc_timing_adv", (double)sc->timing_advance);
    }

    /* ── Neighbor cells (JSON string metric) ──────────────────────── */
    if (cells->ncells_count > 0 && cells->neighbor_cells) {
        /* Buffer: each neighbor ~30 chars + overhead. 17 neighbors max = ~600 bytes */
        char nbuf[640];
        if (serialize_neighbors(cells->neighbor_cells,
                                cells->ncells_count,
                                nbuf, sizeof(nbuf))) {
            conexio_cloud_send_metric_str("_loc_neighbors", nbuf);
            LOG_DBG("cell_location: queued %u neighbor cells", cells->ncells_count);
        }
    }
...
static void lte_lc_evt_handler(const struct lte_lc_evt *const evt)
{
    ...
    if (cells->current_cell.id == LTE_LC_CELL_EUTRAN_ID_INVALID) {
        LOG_WRN("cell_location: neighbor cell measurement failed or timed out");
        return;
    }
...
int cell_location_request(void)
{
    ...
    /* LTE_LC_NEIGHBOR_SEARCH_TYPE_DEFAULT: searches all supported bands.
     * Takes ~1–5 seconds. Finds up to 17 neighbor cells. */
    struct lte_lc_ncellmeas_params params = {
        .search_type = LTE_LC_NEIGHBOR_SEARCH_TYPE_DEFAULT,
        .gci_count   = 0,   /* not used with DEFAULT search type */
    };
