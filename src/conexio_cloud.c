...
#include "cbor_encoder.h"            /* CBOR payload builder (zcbor-based) */
#endif
...
#if defined(CONFIG_CONEXIO_CLOUD_CBOR)
/* ── build_cbor_payload_for_category ─────────────────────────────────────
 *
 * CBOR-encoded alternative to build_payload_for_category().
 * Produces a self-describing CBOR map with the same structure as the JSON
 * payload but encoded in binary — typically 50–70% smaller.
 *
 * Wire format (CBOR map at top level):
 *   {
 *     "dev_id"  : tstr,   -- 15-digit IMEI
 *     "ts"      : tstr,   -- ISO-8601 UTC timestamp
 *     "seq"     : uint,   -- per-topic monotonic sequence number
 *     "topic"   : tstr,   -- "telemetry" | "diagnostics" | "location" | "logs"
 *     "metrics" : {       -- nested map of all metric key-value pairs
 *       "_rssi"    : int,
 *       "temp"     : float16,
 *       ...
 *     }
 *   }
 *
 * @param category  TOPIC_CAT_* constant selecting which topic to build for.
 * @param buf       Caller-provided buffer to write the CBOR bytes into.
 * @param buf_size  Size of buf in bytes.
 * @param out_len   Set to the number of bytes written on success.
 * @return          0 on success, -ENOMEM if buf is too small, -EIO on encode error.
 *
 * This function uses the same static guard variables as build_payload_for_category
 * (s_boot_sent, s_slow_tick) to maintain consistent publish-frequency behaviour
 * regardless of which encoder is active. Both functions share the same boot/slow
 * state — only one should be called per publish cycle (enforced by Kconfig toggle).
 */
static int build_cbor_payload_for_category(char category,
					    uint8_t *buf,
					    size_t   buf_size,
					    size_t  *out_len)
{
    ...
    if (date_time_now(&unix_ms) == 0) {
        ...
        LOG_WRN("CBOR: NTP not synced — timestamp is epoch (1970)");
    }

    /* ── Sequence number and topic name ──────────────────────────────── */
    uint32_t *seq_ptr;
    ...
    uint32_t seq_val = (*seq_ptr)++;

    /* ── Count metrics to pre-declare map size ───────────────────────────
     * zcbor requires knowing the number of key-value pairs before encoding
     * the map. We use a generous upper bound — zcbor will accept fewer entries
     * as long as we call zcbor_map_end_encode() before the map is full.
     * For the outer envelope we use 5 fixed keys (dev_id, ts, seq, topic, metrics).
     * For the metrics map we estimate based on category. */
    #define ENVELOPE_PAIRS  5   /* dev_id + ts + seq + topic + metrics    */
    #define METRIC_PAIRS_MAX 48 /* upper bound — diagnostics boot payload  */

    /* zcbor state: nesting depth = 2 (outer map + metrics map) */
    ZCBOR_STATE_E(zse, 2, buf, buf_size, ENVELOPE_PAIRS);

    cbor_encoder_t enc = CBOR_ENCODER_INIT(zse);

    /* Start outer map */
    if (!zcbor_map_start_encode(zse, ENVELOPE_PAIRS)) return -ENOMEM;

    /* Envelope fields */
    enc = cbor_add_tstr(enc, "dev_id", g_device_id);
    enc = cbor_add_tstr(enc, "ts",     timestamp);
    enc = cbor_add_uint32(enc, "seq",  seq_val);
    enc = cbor_add_tstr(enc, "topic",  topic_name);

    if (!enc.ok) return -ENOMEM;

    /* Start metrics map */
    enc = cbor_start_map(enc, "metrics", METRIC_PAIRS_MAX);
    if (!enc.ok) return -ENOMEM;

    /* ── DIAGNOSTICS: SDK system metrics ────────────────────────────── */
    if (category == TOPIC_CAT_DIAGNOSTICS) {

    static struct modem_param_info cached_modem_param_cbor;
    static int modem_refresh_counter_cbor = 0;

    if (modem_refresh_counter_cbor == 0) {
        if (modem_info_params_get(&cached_modem_param_cbor) != 0) {
            goto skip_modem_metrics_cbor;
        }
    }
    modem_refresh_counter_cbor = (modem_refresh_counter_cbor + 1) %
    ...
    /* Signal quality */
    {
        ...
        if (g_cached_rsrp_idx != RSRP_CACHE_INVALID) {
            ...
            rsrp_val = (int)cached_modem_param_cbor.network.rsrp.value;
        }
    ...
    }
    {
        ...
        if (modem_info_get_snr(&snr_val) == 0 && snr_val != SNR_UNAVAILABLE) {
            enc = cbor_add_int32(enc, "_snr", snr_val);
        }
    }

skip_modem_metrics_cbor:;

#if defined(CONFIG_CONEXIO_CLOUD_PSM)
    {
        int temp = power_mgr_get_modem_temp();
        if (temp != -999) {
            enc = cbor_add_int32(enc, "_modem_temp", temp);
        }
    }
#endif
    ...
    /* Data usage */
    {
        ...
        if (modem_info_get_connectivity_stats(&tx_kb, &rx_kb) == 0) {
            enc = cbor_add_int32(enc, "_tx_kb", tx_kb);
            enc = cbor_add_int32(enc, "_rx_kb", rx_kb);
        }
    }

    /* Boot/slow/every tiers — shared state with JSON builder */
    static bool    s_cbor_boot_sent  = false;
    static uint16_t s_cbor_slow_tick = 0;
    const  uint16_t slow_interval    = CONFIG_CONEXIO_CLOUD_SLOW_METRIC_INTERVAL;

    bool emit_boot = !s_cbor_boot_sent;
    bool emit_slow = (s_cbor_slow_tick == 0);

    if (emit_boot) {
        enc = cbor_add_uint32(enc, "_reboot_cnt", g_reboot_cnt);
        enc = cbor_add_tstr(enc,   "_reboot_reason", g_reboot_reason);

        const struct conexio_lte_session_metrics *lm =
            conexio_lte_get_session_metrics();
        if (lm->connect_time_ms > 0) {
            enc = cbor_add_uint32(enc, "_lte_connect_ms",
                                  (uint32_t)lm->connect_time_ms);
        }
        if (lm->lte_mode != 0) {
            enc = cbor_add_int32(enc, "_lte_mode", lm->lte_mode);
        }
        if (lm->psm_tau_sec >= 0) {
            enc = cbor_add_int32(enc, "_psm_tau_sec", lm->psm_tau_sec);
        }
        if (lm->psm_active_time_sec >= 0) {
            enc = cbor_add_int32(enc, "_psm_active_sec",
                                 lm->psm_active_time_sec);
        }
        if (lm->edrx_interval_ms > 0) {
            enc = cbor_add_uint32(enc, "_edrx_ms",
                                  (uint32_t)lm->edrx_interval_ms);
        }
        if (lm->edrx_ptw_ms > 0) {
            enc = cbor_add_uint32(enc, "_edrx_ptw_ms",
                                  (uint32_t)lm->edrx_ptw_ms);
        }

        /* Firmware versions — boot-once strings */
        {
            static char modem_fw_cbor[MODEM_INFO_FWVER_SIZE] = {0};
            if (modem_fw_cbor[0] == '\0') {
                modem_info_get_fw_version(modem_fw_cbor,
                                          sizeof(modem_fw_cbor));
            }
            if (modem_fw_cbor[0] != '\0') {
                enc = cbor_add_tstr(enc, "_mfw", modem_fw_cbor);
            }
        }
    ...
    /* _op — boot-once but retried until attached */
    {
        static bool s_cbor_op_sent = false;
        if (!s_cbor_op_sent) {
            char op_buf[MODEM_INFO_SHORT_OP_NAME_SIZE] = {0};
            if (modem_info_get_operator(op_buf, sizeof(op_buf)) == 0
                && op_buf[0] != '\0') {
                enc = cbor_add_tstr(enc, "_op", op_buf);
                s_cbor_op_sent = true;
            }
        }
    }

    /* Delta metrics: _lte_band, _cell_id, _tac */
    {
        static uint8_t  s_cbor_prev_band    = 0xFF;
        static uint32_t s_cbor_prev_cell_id = 0xFFFFFFFF;
        static uint16_t s_cbor_prev_tac     = 0xFFFF;
        ...
        if (modem_info_get_current_band(&band) == 0
            ...
            if (emit_boot || emit_slow || band != s_cbor_prev_band) {
                enc = cbor_add_int32(enc, "_lte_band", band);
                s_cbor_prev_band = band;
            }
        }
        if (lm_slow->cell_id != 0xFFFFFFFF) {
            if (emit_boot || emit_slow
                || lm_slow->cell_id != s_cbor_prev_cell_id) {
                enc = cbor_add_uint32(enc, "_cell_id", lm_slow->cell_id);
                s_cbor_prev_cell_id = lm_slow->cell_id;
            }
        }
        if (lm_slow->tac != 0xFFFFFFFF) {
            uint16_t tac = (uint16_t)lm_slow->tac;
            if (emit_boot || emit_slow || tac != s_cbor_prev_tac) {
                enc = cbor_add_uint32(enc, "_tac", tac);
                s_cbor_prev_tac = tac;
            }
        }
    }

    s_cbor_slow_tick = (s_cbor_slow_tick + 1) % slow_interval;
    if (emit_boot) {
        s_cbor_boot_sent = true;
    }

    /* EVERY-PUBLISH diagnostics metrics */
    {
        const struct conexio_lte_session_metrics *lm =
            conexio_lte_get_session_metrics();
        if (lm->reset_loop_detected) {
            enc = cbor_add_int32(enc, "_reset_loop", 1);
        }
        enc = cbor_add_uint32(enc, "_conn_loss",
                              (uint32_t)lm->connection_loss_count);
    }

    if (g_publish_success_count > 0 || g_publish_fail_count > 0) {
        enc = cbor_add_uint32(enc, "_pub_ok", g_publish_success_count);
        if (g_publish_fail_count > 0) {
            enc = cbor_add_uint32(enc, "_pub_fail", g_publish_fail_count);
        }
    }
...
#if defined(CONFIG_CONEXIO_CLOUD_BATTERY_METRICS)
    {
        ...
        if (soc >= 0.0f) {
            ...
            if (!is_charging && g_last_soc_pct >= 0.0f
                ...
                float elapsed_hr =
                    (float)(now_ms - g_last_pub_time_ms) / 3600000.0f;
                if (elapsed_hr > 0.0f) {
                    float drain = (g_last_soc_pct - soc) / elapsed_hr;
                    if (drain > 0.0f) {
                        enc = cbor_add_float32(enc, "_batt_drain_hr", drain);
                    }
                }
            }
            g_last_soc_pct     = soc;
            g_last_pub_time_ms = k_uptime_get();
        }
    }
#endif
...
#if defined(CONFIG_CONEXIO_CLOUD_AUTO_BATTERY)
    {
        ...
        if (modem_info_get_batt_voltage(&bat_mv) == 0 && bat_mv > 0) {
            enc = cbor_add_uint32(enc, "_batt_mv", (uint32_t)bat_mv);
        }
    }
#endif
    ...
    /* ── TELEMETRY: sensor registry + metric queue ───────────────────── */
    if (category == TOPIC_CAT_TELEMETRY) {
        for (int i = 0; i < sensor_count; i++) {
            ...
            if (!isnan(val)) {
                enc = cbor_add_float64(enc, sensor_registry[i].name, val);
            }
        }
    }

    /* ── Queued application metrics (all non-logs categories) ────────── */
    if (category != TOPIC_CAT_LOGS) {
        ...
        for (int i = 0; i < CONFIG_CONEXIO_CLOUD_METRIC_QUEUE_SIZE; i++) {
            ...
            case 'n':
                enc = cbor_add_float64(enc, metric_queue[i].name,
                                       metric_queue[i].num_val);
                break;
            case 's':
                enc = cbor_add_tstr(enc, metric_queue[i].name,
                                    metric_queue[i].str_val);
                break;
            case 'b':
                enc = cbor_add_bool(enc, metric_queue[i].name,
    ...
    /* Close metrics map */
    enc = cbor_end_map(enc, METRIC_PAIRS_MAX);

    /* Close outer envelope map */
    if (enc.ok) {
        enc.ok = zcbor_map_end_encode(zse, ENVELOPE_PAIRS);
    }

    if (!enc.ok) {
        LOG_ERR("CBOR encode failed for category '%c' — buffer too small?",
                category);
        return -EIO;
    }

    *out_len = (size_t)(zse->payload - buf);
    LOG_DBG("CBOR payload: category='%c' len=%zu", category, *out_len);
    return 0;
}
...
int conexio_cloud_init(conexio_cloud_event_cb_t cb)
{
    ...
    !(defined(CONFIG_CONEXIO_CLOUD_INGEST_KEY) && (sizeof(CONFIG_CONEXIO_CLOUD_INGEST_KEY) > 1))
...
int conexio_cloud_publish_single(char category)
{
    ...
    /* Location topic always publishes as JSON regardless of CBOR setting.
     * _loc_* metrics include variable-length string fields (_loc_nbrs) and
     * are processed by a dedicated cloud Lambda (IotDashboardLocationRule)
     * that expects JSON. No binary location rule exists on the cloud side. */
#if defined(CONFIG_CONEXIO_CLOUD_CBOR)
    if (category != TOPIC_CAT_LOCATION) {
        ...
        int cbor_ret = build_cbor_payload_for_category(category,
                                                        ...
                                                        &cbor_len);
        if (cbor_ret != 0) {
            LOG_ERR("CBOR build failed for category '%c': %d",
                    category, cbor_ret);
            return cbor_ret;
        }
        return transport_publish_to(category,
                                    (const char *)cbor_buf, cbor_len);
    }
#endif
    /* ── JSON publish path — location always, other categories when CBOR off */
...
int conexio_cloud_publish(void)
{
    ...
    for (int c = 0; c < (int)sizeof(categories); c++) {
        ...
        if (cat != TOPIC_CAT_LOCATION) {
            ...
            int ret = transport_publish_to(cat,
                                           (const char *)cbor_buf,
                                           cbor_len);
            if (ret != 0) {
                LOG_WRN("CBOR publish failed for category '%c' (%d)",
                        cat, ret);
                overall = ret;
            }
