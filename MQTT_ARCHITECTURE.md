...
## Device Provisioning & Certificate Architecture

### Overview

Every Conexio Stratus device goes through a one-time **AWS Fleet Provisioning**
flow before it can send telemetry. This produces a unique, hardware-bound identity
for each device that cannot be cloned or transferred.

### Provisioning Flow (`samples/conexio-stratus-provision`)

```
AT%KEYGEN  →  generates key pair inside modem secure element
              (private key never leaves modem, never in RAM or logs)
              returns a CSR (Certificate Signing Request)
                    ↓
CreateCertificateFromCsr  →  sends CSR to AWS IoT Core via MQTT
              (using shared claim credentials)
              AWS signs the CSR → returns device-unique certificate (~1.7 KB)
              Note: CreateKeysAndCertificate is NOT used — its response is
              ~3.6 KB which overflows the nRF91xx modem TLS receive buffer.
              CSR-based flow keeps the response within the 2 KB buffer limit.
                    ↓
RegisterThing  →  publishes ownership token + IMEI to fleet provisioning template
              AWS creates a Thing named after the IMEI in IoT Registry
              IoT policy attached → device can now publish/subscribe
                    ↓
cert_store_write_device_creds()  →  writes device certificate to modem TLS tag 21
              Private key already at tag 21 (generated by AT%KEYGEN)
                    ↓
Settings NVS: persist "prov/done=true", flush, modem shutdown
                    ↓
sys_reboot()  →  device reboots and runs main app with its own identity

```

### After Provisioning — What Each Device Has

| | Claim Phase | After Provisioning |
|---|---|---|
| **Identity** | Shared bootstrap cert (`claim.crt`) | Unique per-device X.509 certificate |
| **Private key** | Shared (`claim.key`) | Unique, hardware-bound in modem secure element |
| **AWS IoT Thing** | None | Entry named after IMEI (e.g. `355025934980275`) |
| **MQTT permissions** | Provisioning APIs only | Full telemetry/commands policy |
| **TLS tags used** | 10 (CA), 11 (cert), 12 (key) | 20 (CA), 21 (cert), 21 (key) |

Specifically, after provisioning each device has:
- Its own **X.509 certificate** in AWS IoT, named after its IMEI (e.g. `355025934980275`)
- Its own **private key locked in the modem's secure element** — cryptographically bound to that hardware, never extractable
- An **entry in the AWS IoT Thing registry** with the device's IMEI as the Thing name

### Shared Credentials (Baked into Firmware)

| File | Purpose | Shared? |
|---|---|---|
| `src/certs/AmazonRootCA1.pem` | AWS root CA — same for all AWS regions | ✓ Always shared |
| `src/certs/claim.crt` | Bootstrap certificate — all devices use this to trigger provisioning | ✓ Yes, by design |
| `src/certs/claim.key` | Bootstrap private key — paired with `claim.crt` | ✓ Yes, by design |

The claim certificate's AWS IoT policy is intentionally minimal:
it can only call `iot:CreateCertificateFromCsr` and `iot:RegisterThing`.
It cannot publish telemetry or access any device data.

### Provisioning TLS Security Tags (modem credential storage)

| Tag | Default | Content | Phase |
|---|---|---|---|
| 10 | `CONFIG_STRATUS_CLAIM_CA_TAG` | Root CA (AmazonRootCA1.pem) | Claim phase |
| 11 | `CONFIG_STRATUS_CLAIM_CERT_TAG` | Shared claim certificate | Claim phase |
| 12 | `CONFIG_STRATUS_CLAIM_KEY_TAG` | Shared claim private key | Claim phase |
| 20 | `CONFIG_STRATUS_DEVICE_CA_TAG` | Root CA (same PEM, device slot) | Normal operation |
| 21 | `CONFIG_STRATUS_DEVICE_CERT_TAG` | **Unique** per-device certificate (AWS-signed) | Normal operation |
| 21 | `CONFIG_STRATUS_DEVICE_KEY_TAG` | **Unique** per-device private key (AT%KEYGEN) | Normal operation |

Tags 10–12 remain in modem storage after provisioning but are never used again.
...
*Last updated: September 2026 — Conexio SDK v2.3.0 / App v1.0.8 / Provision v1.0.0*
